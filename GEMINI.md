...
* `docs/ai/greenfield-generator-mechanics.md` is the design for generating `_types.go` mechanically instead of by hand: `+required` from `field_behavior`, parent shape from `google.api.resource`, Spec pre-population, and a per-service `needs_judgement_call.txt` queue for what an AI agent or a human still has to decide. Implemented in PRs #14-#17, none merged yet; two of the four are opt-in per service. (Sandbox-only.)
* `docs/ai/greenfield-generator-findings.md` holds the evidence behind that design - annotation coverage, what emitting `+required` actually changes, how often the identity template's guess is wrong - plus the approaches that were measured and abandoned. Read it when you want to know why a phase is gated, or before measuring a generator change - it records why the checked-in tree is not a valid baseline. (Sandbox-only.)
* `docs/ai/greenfield-quality-enforcement.md` explains how the greenfield apichecks are designed and why: the difference between a golden baseline and a ratchet, scoping a check to the bulk manifest, the four states a reference finding can be in, and how to choose a source of truth a check can actually run against in CI. Read it before adding or changing a greenfield check. (Sandbox-only.)
* `docs/ai/experiments/identity-collection-casing.md` records nine `_identity.go` files whose GCP collection segment is lowercased where the API uses camelCase, the recorded-traffic oracle that proves it, and what the PR #18 check does and does not cover. (Sandbox-only.)
