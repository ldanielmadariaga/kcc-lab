...
* `docs/ai/greenfield-step1-workflow.md` is the runbook for Step 1 under the experimental flags: the commands that take a resource from nothing to a complete `_types.go` and CRD, what each stage produces and where it lands, and the gaps where the pipeline still stops short. Follow it to generate types and a CRD. (Sandbox-only.)
