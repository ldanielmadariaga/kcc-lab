...
* `docs/ai/greenfield-quality-enforcement.md` explains how the apichecks for greenfield work and why: golden vs ratchet baselines, manifest scoping, the four states a reference finding can be in, and how to pick an oracle a check can actually run against. Read it before adding or changing a greenfield check. (Sandbox-only.)
