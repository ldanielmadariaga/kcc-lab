...
func CCInsightsConversationObservedState_FromProto(mapCtx *direct.MapContext, in *pb.Conversation) *krm.CCInsightsConversationObservedState {
	...
	if in.DialogflowIntents != nil {
		out.DialogflowIntents = make(map[string]krm.DialogflowIntent, len(in.DialogflowIntents))
		for k, v := range in.DialogflowIntents {
			if c := DialogflowIntent_FromProto(mapCtx, v); c != nil {
				out.DialogflowIntents[k] = *c
			}
		}
	}
	return out
...
func CCInsightsConversationObservedState_ToProto(mapCtx *direct.MapContext, in *krm.CCInsightsConversationObservedState) *pb.Conversation {
	...
	if in.DialogflowIntents != nil {
		out.DialogflowIntents = make(map[string]*pb.DialogflowIntent, len(in.DialogflowIntents))
		for k, v := range in.DialogflowIntents {
			out.DialogflowIntents[k] = DialogflowIntent_ToProto(mapCtx, &v)
		}
	}
	return out
}
