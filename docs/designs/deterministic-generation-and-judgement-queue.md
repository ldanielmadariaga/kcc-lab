...
- It does not change upstream's conventions. This work lives in the kcc-lab sandbox repository.
...
We tested the design by deleting resources that upstream wrote by hand, regenerating them from the
proto, and comparing the result with upstream's version, which serves as the known-good answer. The
figures below come from the experiment branch's `greenfield-state-of-play.md`,
`greenfield-coverage-invariant.md` and `greenfield-step1-workflow.md`. They were measured with the
generator on PR #21, before the work was split into the PRs listed in section 7, so they are
evidence for the approach rather than a measurement of master.

- Over a 275-resource corpus, 91.4% of upstream's fields came out as upstream has them, 3.1% came
  out differently, and 5.5% were not produced at all.
...
- With only `google.api.resource_reference` to go on, the queue named 11 of the 111 reference fields
  a 239-resource run needed. With the reference rules applied as well, first by the second-pass
  `scripts/queue-hints` and now by `--emit-reference-hints`, it named 82 of 111.
...
- Everything else: #53 (empty ObservedState), #59 (duplicate GVK), #62 (location).
