...
Two things this document states less plainly than it should, found while running the larger
replication ([replication-239.md](replication-239.md)):

The 40 scored resources were **40 of 100**, not 40 of some smaller candidate set. The funnel was:
100 candidates, 5 out of scope, 9 lost to a bug in the measurement harness, 46 lost to
whole-service quarantine after a compile failure, 40 scored.

Those 9 were **the harness, not the generator**. It resolved a Kind's CRD by assuming the filename
was `lowercase(kind)+"s"`, which is wrong for 71 of 615 kinds: `*Policy` becomes `policies`, `Batch`
becomes `batches`, `Process` becomes `processes`. Every one of those CRDs existed on disk and simply
could not be found. The later run resolves CRDs from each file's own `kind:` field and loses none.

The 46 are discussed in the newer document. Quarantining by *service* meant one broken resource
removed its package-mates with it, so `networksecurity` lost all 12 of its resources at once. The
scored count was gated by package blast radius rather than by resource quality, which also means
the 98.6% adjusted spec figure below is measured on survivors — the easy cases, since anything hard
had already been quarantined away. The 239-resource run reports 86.1% on the same basis.
