...
// invocation's own resource list is always available but covers only its slice
// of a service generated by several generate-types calls -- discoveryengine
// twice, dialogflow four times. The package scan covers the whole service, but
// a wipe-based regeneration deletes every _types.go before the generator runs,
// so on that path it finds nothing at all. Union of the two.
//
// The key strips the service prefix: DiscoveryEngineDataStore is keyed
// "datastore", so a field called dataStore matches it.
