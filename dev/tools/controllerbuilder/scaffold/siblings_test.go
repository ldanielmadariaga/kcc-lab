...
func TestSiblingResources(t *testing.T) {
	grid := []struct {
		...
		{
			// A service generated by several generate-types calls has Kinds in
			// this run that the package does not declare yet, and the other way
			// round. Neither source is complete, so both are read.
		...
		{
			// A Kind from another service names nothing this service declares,
			// and keying it under its own full name would match field names
		...
		{
			// A wipe-based regeneration deletes every _types.go before the
			// generator runs, so on that path the scan finds nothing and this
			// run's own list is all there is.
