...
func (o *GenerateCRDOptions) BindFlags(cmd *cobra.Command) {
	...
	cmd.Flags().BoolVar(&o.EmitRequiredFromProto, "emit-required-from-proto", false, "emit // +required for fields the proto marks REQUIRED. Opt-in per service: enabling it for an existing resource can tighten its CRD schema, because nested types are shared between spec and status")
}
