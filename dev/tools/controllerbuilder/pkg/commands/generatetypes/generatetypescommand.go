...
func (o *GenerateCRDOptions) BindFlags(cmd *cobra.Command) {
	...
	cmd.Flags().BoolVar(&o.EmitSiblingRefs, "emit-sibling-refs", false, "mark a string field whose name matches a resource this service declares as a probable reference to it, with a +kcc:guess comment and an entry in apis/<service>/needs_judgement_call.txt. The field stays a string: this reports a candidate rather than generating a reference. Opt in one service at a time, though the comment is stripped from the CRD so it cannot change the schema")
}
...
func RunGenerateCRD(ctx context.Context, o *GenerateCRDOptions) error {
	...
	// The sibling map is built from two sources because neither is complete on
	// its own: this run's own resource list covers only its slice of a service
	// generated by several generate-types calls, and the package scan covers the
	// whole service but finds nothing after a wipe-based regeneration, which
	// deletes every _types.go before the generator runs.
	//
	// Computed here rather than beside the scaffolding loop because
	...
	if o.PrepopulateSpec {
		...
		// They are reported per service and written as comments, not entries.
		// A nested message is shared by every resource that references it, so
		// there is no single Kind to attribute a field to. Every non-comment line
		// in this file suppresses [refs] for the Kind it names, so a made-up Kind
		// would quietly switch off a real check.
	...
	// same reason the dropped fields above are: a nested message is shared by
	// every resource that references it, so there is no single Kind to attribute
	// one to, and every non-comment line in this file suppresses [refs] for the
	// Kind it names.
