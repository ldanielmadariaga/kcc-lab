...
func (o *GenerateCRDOptions) BindFlags(cmd *cobra.Command) {
	...
	cmd.Flags().BoolVar(&o.EmitMixedCaseAcronyms, "emit-mixed-case-acronyms", false, "spell an acronym as the Acronyms list spells it, so boot_disk_mib becomes bootDiskMiB rather than bootDiskMIB. Affects only MiB and OAuth2; opt in one service at a time, since it renames fields")
...
func RunGenerateCRD(ctx context.Context, o *GenerateCRDOptions) error {
	...
	writeOptions := codegen.WriteOptions{
		...
		EmitMixedCaseAcronyms: o.EmitMixedCaseAcronyms,
